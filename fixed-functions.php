...
function validateAppointmentRequest(PDO $pdo, int $userId, $serviceId, $date, $time, ?int $excludeId = null): array
{
    ...
    // Skip user validation for guests ($userId <= 0)
    if ($userId > 0) {
...
function getAdminDashboardStats(PDO $pdo): array
{
    ...
    $stmt = $pdo->prepare(
        "SELECT
            ...
            SUM(CASE WHEN status = 'approved' THEN 1 ELSE 0 END) AS approved,
            SUM(CASE WHEN status = 'completed' THEN 1 ELSE 0 END) AS completed,
            SUM(CASE WHEN appointment_date = ? THEN 1 ELSE 0 END) AS today_total
         FROM appointments"
    );
    $stmt->execute([$today]);
    $stats = $stmt->fetch() ?: [];
...
{
    if ($period === 'daily') {
        $sql = "SELECT DATE_FORMAT(appointment_date, '%Y-%m-%d') AS label, COUNT(*) AS total
                FROM appointments
                GROUP BY appointment_date
                ORDER BY appointment_date DESC
                LIMIT 30";
    } elseif ($period === 'weekly') {
        $sql = "SELECT CONCAT(YEAR(appointment_date), '-W', LPAD(WEEK(appointment_date, 1), 2, '0')) AS label, COUNT(*) AS total
                FROM appointments
                GROUP BY YEAR(appointment_date), WEEK(appointment_date, 1)
                ORDER BY YEAR(appointment_date) DESC, WEEK(appointment_date, 1) DESC
                LIMIT 12";
    } else {
        $sql = "SELECT DATE_FORMAT(appointment_date, '%Y-%m') AS label, COUNT(*) AS total
                FROM appointments
                GROUP BY DATE_FORMAT(appointment_date, '%Y-%m')
                ORDER BY DATE_FORMAT(appointment_date, '%Y-%m') DESC
                LIMIT 12";
    }
...
function createWalkInClient(PDO $pdo, string $first_name, string $last_name, ?string $email, string $mobile, ?int $age = null): array
{
    $errors = [];
