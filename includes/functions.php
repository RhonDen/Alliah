...
function toPositiveInt($value): ?int
{
...
function isValidAppointmentStatus(string $status): bool
{
    return in_array($status, APPOINTMENT_STATUSES, true);
}

function isActiveAppointmentStatus(string $status): bool
{
...
function isSlotInPast(string $date, string $time): bool
{
...
function getServiceById(PDO $pdo, int $serviceId): ?array
{
...
function getTimeSlots(int $intervalMinutes = APPOINTMENT_INTERVAL_MINUTES): array
{
    $slots = [];
...
function getBookedTimeSlots(PDO $pdo, string $date, ?int $excludeId = null): array
{
...
function getAvailableTimeSlots(PDO $pdo, string $date, ?int $excludeId = null): array
{
...
function isSlotAvailable(PDO $pdo, string $date, string $time, ?int $excludeId = null): bool
{
    $normalizedDate = normalizeDate($date);
    $normalizedTime = normalizeTime($time);
    if ($normalizedDate === null || $normalizedTime === null || !isValidTimeSlot($normalizedTime)) return false;
    if (isSlotInPast($normalizedDate, $normalizedTime)) return false;
...
function validateAppointmentRequest(PDO $pdo, int $userId, $serviceId, $date, $time, ?int $excludeId = null): array
{
    $errors = [];
    ...
    if ($userId > 0) {
        ...
        $stmt->execute([$userId]);
        if (!$stmt->fetchColumn()) $errors[] = 'Please select a valid patient account.';
    }
...
function createAppointment(PDO $pdo, int $userId, $serviceId, $date, $time, string $status = 'pending', ?string $adminMessage = null): array
{
    $validation = validateAppointmentRequest($pdo, $userId, $serviceId, $date, $time);
    if (!isValidAppointmentStatus($status)) $validation['errors'][] = 'Invalid appointment status.';
...
function createWalkInAppointment(PDO $pdo, int $userId, $serviceId, $date, $time): array
{
    return createAppointment($pdo, $userId, $serviceId, $date, $time, 'approved');
}

function getUser(PDO $pdo, ?int $userId = null): ?array
{
...
function getUserAppointments(PDO $pdo, int $userId, array $filters = []): array
{
...
function getUserAppointmentStats(PDO $pdo, int $userId): array
{
    $today = date('Y-m-d');
    $now = date('H:i:s');
    $stmt = $pdo->prepare(
        ...
        FROM appointments WHERE user_id = ? AND status != 'rejected'"
    );
...
function getAllAppointments(PDO $pdo, array $filters = []): array
{
...
function getAppointmentById(PDO $pdo, int $appointmentId): ?array
{
...
function canTransitionAppointmentStatus(string $currentStatus, string $newStatus): bool
{
    $allowedTransitions = [
...
function updateAppointmentStatus(PDO $pdo, int $appointmentId, string $status, ?string $adminMessage = null): array
{
    $appointment = getAppointmentById($pdo, $appointmentId);
    if ($appointment === null) return ['success' => false, 'errors' => ['Appointment not found.']];
    if (!isValidAppointmentStatus($status)) return ['success' => false, 'errors' => ['Please choose a valid appointment status.']];
    if (!canTransitionAppointmentStatus($appointment['status'], $status)) return ['success' => false, 'errors' => ['That appointment status change is not allowed.']];
    if (isActiveAppointmentStatus($status) && !isActiveAppointmentStatus($appointment['status']) && !isSlotAvailable($pdo, $appointment['appointment_date'], $appointment['appointment_time'], $appointmentId)) {
    ...
    return ['success' => $updated, 'errors' => $updated ? [] : ['The appointment could not be updated.']];
}
...
function getRecentAppointments(PDO $pdo, int $limit = 5): array
{
...
function getMostBookedServices(PDO $pdo, int $limit = 5): array
{
    $safeLimit = max(1, (int) $limit);
    $stmt = $pdo->query(
...
function getPeakTimes(PDO $pdo): array
{
...
function getMonthlyComparison(PDO $pdo): array
{
    $current = date('Y-m');
